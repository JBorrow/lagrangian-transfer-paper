...
own = np.load("baryon_fraction_vs_stellar_mass_own.npy")
other = np.load("baryon_fraction_vs_stellar_mass_other.npy")
outside = np.load("baryon_fraction_vs_stellar_mass_outside.npy")
halo_mass = get_fancy_halo_mass(np.load("baryon_fraction_stellar_masses.npy"))
...
for (label, name), c in zip(switch.items(), colors):
    ...
    ax.fill_between(
        halo_mass,
        sum(previous), sum(previous) + current_data,
        color=c,
        label=label,
        alpha=0.6,
        linewidth=0,
...
ax.plot(halo_mass, sum(previous), color="black", label="Total")
...
fig.tight_layout()
fig.savefig("baryon_fraction_breakdown_stellar.pdf")
