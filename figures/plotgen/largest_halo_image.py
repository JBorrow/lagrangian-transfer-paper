...
#%% [markdown]
# Something people have been talking about for a while is to compare this with the
# metallicity of the particles. Let's take a look at that.

#%%
with h5py.File(data.snapshot_end.snapshot_filename, "r") as handle:
    ids = np.argsort(handle["PartType0/ParticleIDs"][...])
    metallicity = handle["PartType0/Metallicity"][...][ids]

#%%
qv_metal_weighted = QuickView(
    gas_coordinates_around_center[mask_in_lr],
    hsml=qv_nomass.get_hsml(),
    mass=metallicity[mask_in_lr],
    **kwargs,
)
